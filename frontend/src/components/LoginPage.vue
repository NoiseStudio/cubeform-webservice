<template>
  <div class="mainRegisterContainer">
    <ValidationObserver tag="form" name="form" @submit.prevent="handleSubmit" v-slot="{invalid}" class="registerServerForm">
      ...
      <ValidationProvider name="password" rules="required|max:255" v-slot="{errors}" class="singleField">
        <div class="nameInput">
...
export default {
  ...
  methods: {
    ...
    loginUser(response){
      let userId = response.jsonBody.userId;
      let token = response.jsonBody.jwt;
      return this.$store.dispatch("User/login", {token: token, id: userId})
          .then(() => {
